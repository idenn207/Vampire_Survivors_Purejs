...
(function(Core) {
  ...
  class Input {
    ...
    /**
     * Update input state (call at end of frame to clear per-frame states)
     */
    update() {
     ...
     * @param {string} key - Key code (e.g., 'KeyW', 'Space', 'ArrowUp')
     * @returns {boolean}
     */
    isKeyDown(key) {
    ...
    _handleKeyDown(e) {
      ...
      if (e.code === 'Tab' || e.code === 'Space' || e.code === 'Escape') {
        e.preventDefault();
      }
    ...
    /** Mouse position in world coordinates (camera-adjusted) */
