...
(function(Core) {
  ...
  class Camera {
    ...
    // Transform component class reference (set by game)
    ...
    /**
     * Set the Transform component class for entity following
     * @param {Function} TransformClass - The Transform component class
     */
     ...
     * @param {Entity} entity - Entity to follow (must have Transform component)
     */
    follow(entity) {
      this._target = entity;
     ...
     */
    update(deltaTime) {
      if (!this._target || !this._isFollowing || !this._TransformClass) return;

      var transform = this._target.getComponent(this._TransformClass);
      if (!transform) return;
      ...
      if (this._worldBounds) {
        this._position.x = Math.max(
          ...
          Math.min(this._worldBounds.maxX - this._viewportWidth, this._position.x)
        );
        this._position.y = Math.max(
          this._worldBounds.minY,
          Math.min(this._worldBounds.maxY - this._viewportHeight, this._position.y)
        );
      }
    }
    ...
    getDebugInfo() {
      var targetName = 'None';
      if (this._target) {
        if (this._target.hasTag && this._target.hasTag('player')) {
          targetName = 'Player';
        } else {
